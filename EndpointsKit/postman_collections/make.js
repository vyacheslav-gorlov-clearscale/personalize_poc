...
const matches = input.matchAll(new RegExp('((?<=\PersonalizeAPIGatewayEndpoint)(.*?= )(.*?)(?=\\n))'))
...
const postmanCollectionString = fs.readFileSync(path.join(__dirname, "PersonalizePOC.postman_collection.json"), {
    encoding: "utf8"
})
const processedPostmanCollectionString = postmanCollectionString.replace("@API_GATEWAY_ENDPOINT", apiEndpoint)

const outputPostmanCollectionDirectory = "/tmp/ClearScale-PersonalizePOC"
